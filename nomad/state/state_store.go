...
func deleteNodeCSIPlugins(txn *memdb.Txn, node *structs.Node, index uint64) error {
	...
	for id := range names {
		...
		plug := raw.(*structs.CSIPlugin)
		plug.DeletePlugins(node.ID, index)
...
func (s *StateStore) csiPluginsByJob(ws memdb.WatchSet, job *structs.Job, index uint64) (map[string]*structs.CSIPlugin, error) {
	...
	for _, tg := range job.TaskGroups {
		for _, t := range tg.Tasks {
			...
			if plug == nil {
				plug = structs.NewCSIPlugin(t.CSIPluginConfig.ID, t.CSIPluginConfig.Driver, index)
				plug.Type = t.CSIPluginConfig.Type
			}

			if plug.Driver == "" {
				plug.Driver = t.CSIPluginConfig.Driver
			}

			if plug.Driver != t.CSIPluginConfig.Driver {
				return nil, fmt.Errorf("cannot update plugin id %s with new driver %s", plug.ID, t.CSIPluginConfig.Driver)
			}
...
func (s *StateStore) CSIVolumeDenormalize(ws memdb.WatchSet, vol interface{}) (*structs.CSIVolume, error) {
	...
	if plug == nil {
		v.ControllerHealthy = 0
		v.NodeHealthy = 0
		v.Healthy = false
		return v, nil
	}

	v.ControllerHealthy = plug.ControllerHealthy
	v.NodeHealthy = plug.NodeHealthy
	...
	v.ControllerExpected = len(plug.Controllers)
	v.NodeExpected = len(plug.Nodes)

	v.Healthy = v.ControllerHealthy > 0 && v.NodeHealthy > 0
...
// CSIPlugins returns the unfiltered list of all plugin health status
func (s *StateStore) CSIPluginsByDriver(ws memdb.WatchSet, driver string) (memdb.ResultIterator, error) {
	txn := s.db.Txn(false)
	defer txn.Abort()

	iter, err := txn.Get("csi_plugins", "driver", driver)
	if err != nil {
		return nil, fmt.Errorf("csi_plugins lookup failed: %s %v", driver, err)
	}
